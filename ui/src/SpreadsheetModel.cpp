...
SpreadsheetModel::SpreadsheetModel(std::shared_ptr<Sheet> s, QObject* parent) : QAbstractTableModel(parent), sheet(s) {}
...
bool SpreadsheetModel::setData(const QModelIndex& index, const QVariant& value, int role) {
    ...
    if (sheet->set_cell(index.column(), index.row(), value.toString().toStdString())) {
        ...
        std::shared_ptr<Cell> cell = sheet->get_cell(index.column(), index.row());
